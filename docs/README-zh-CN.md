...
**小米焦点触控笔（二代 / Pro）笔身手势**
- 在「设置 → 手写笔设置 → 手写笔按键方案」选择「小米焦点触控笔」，即可分别为 **轻捏、连击、上滑、下滑** 指定动作（切换颜色、橡皮、撤销、粗细、上一支笔等）。映射只在本应用内生效，不会读写系统的手写笔设置。
- 原理：画布显示期间，应用通过随包的 PenEngine SDK 向 HyperOS 手写笔服务（`com.xiaomi.touchservice`）登记并启用「触控按键」模式，系统随后把笔身手势作为 KeyEvent 194–197 送到画布窗口，而不是弹出系统快捷轮盘。画布隐藏或开启透传时会自动解除登记，把手势还给系统。
- 「系统握手状态」一行会显示是否连接成功；打开「按键诊断」后，画布每收到一个按键事件就会弹出提示，便于排查。无需小米开发中心的 App ID。

**屏幕背景保存**
- 「记住屏幕截图授权」开启时，同一次运行内只需同意一次「允许应用投射」；Android 14+ 的授权只能用一次，所以应用会持有截图会话直到服务停止（期间状态栏会有投屏图标）。Android 14+ 上授权对话框只提供「整个屏幕」。
