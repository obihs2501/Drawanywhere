...
- 「系统握手状态」一行显示连接与 `setEnable` 返回值；打开「按键诊断」后画布每收到一个笔按键就弹出提示，用于排查。

**构建与发布**
- 每次推送都会在 GitHub Actions 构建 debug 与 release APK（产物 `drawanywhere-debug-apk` / `drawanywhere-release-apk`）；推送 `v*` 标签会自动创建 GitHub Release 并附上两个 APK。
- release 默认用仓库内的 `app/ci-release.keystore` 签名（无秘密可言，仅保证同一密钥可覆盖安装）；正式发布请在 `local.properties` 里配置自己的密钥。debug 与 release 签名不同，二者之间切换需先卸载。
